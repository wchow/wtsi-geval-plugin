...
#warn $slice->name,"\t$species\t$alignment_type\t$seq_region_name";
  ...
  $| =1 ;
  # wc2 - Update the distance to 150k compared to 10k before.
  my $max_distance_for_clustering = 150000; #10000;
  ...
  if ($based_on_group_id) {
    ...
    foreach my $name_strand (keys %name_strand_clusters) {
      ...
      foreach my $block (sort {$a->hstart <=> $b->hstart} @{$name_strand_clusters{$name_strand}||[]}) {
        unless (scalar @sub_clusters) {
          push @sub_clusters, [$block->hseqname,$block->hstrand, $block->hstart, $block->hend, $block->seq_region_start, $block->seq_region_end, ($block->hend - $block->hstart), 1];
          next;
        }
        ...
        foreach my $arrayref (@sub_clusters) {
          ...
          if ($block->hstart<=$e &&
            ...
            $arrayref->[7]++;
	    $arrayref->[6] += ($block->hend - $block->hstart);
            $block_clustered = 1;
    ...
    # wc2 Changed to sort by the size of the hit then the number of blocks in cluster.  This was done to reflect
    #  the large hits created from similar assembly compara analysis (eg HumanPGP to HumanAGP with similar timestamps).
    #  Because of this, the default would yield to the quantity of small hits on another chr/region versus the one single
    #  large hit.  wc2---.
    #      OLD:  @refined_clusters = sort {$b->[-1] <=> $a->[-1]} @refined_clusters;
   
    @refined_clusters = sort {  $b->[6] <=> $a->[6] || 
				    ($b->[3] -$b->[2]) <=> ($a->[3] - $a->[2]) ||  
				    $b->[-1] <=> $a->[-1]} @refined_clusters;
    
    
    # wc2.
    my $containerSize = $slice->end - $slice->start;

    my ($altered_start, $altered_end) = ($refined_clusters[0]->[2], $refined_clusters[0]->[3]);
    if ($slice->start > $refined_clusters[0]->[4] ){ #&& $slice->end < $refined_clusters[0]->[5]){
	$altered_start = $refined_clusters[0]->[2] + ($slice->start - $refined_clusters[0]->[4] + 1);
    }	
    # The midpt calcuation doesn't work well with large large large hit regions "think tens of MB's" which again only occurs with similar
    #  assembly compara analysis.  Because of this, I'm limiting the midpt to the size of the query container slice/2 + starting pt.  The
    #  only pt this may cause issues are differing gapped region between these similar regions, however this can be solved by dragging the
    #  the target window bigger.  Because the alignments are so large, there isn't really a way to figure out the end bit coord in the target. 

    #my $midpt         = ( 1000000 < ($refined_clusters[0]->[3] - $refined_clusters[0]->[2]) ) ? 
    #						  ( $altered_start + ($containerSize/2) ) :
    #	
    my $midpt    = ( $altered_start + ($containerSize/2) );

### wc2 note ###
# For testing purposes, turn it on if needed.
if (0){
  for(my $fi=0;$fi<@refined_clusters;$fi++){
    warn "\nrefined__cluster: ". join ("\n", $refined_clusters[$fi]->[0], 
	    $refined_clusters[$fi]->[2]+ int(($refined_clusters[$fi]->[3] - $refined_clusters[$fi]->[2])/2),
            $refined_clusters[$fi]->[1] * $slice->strand,
            $refined_clusters[$fi]->[2] ."vs $altered_start",
            $refined_clusters[$fi]->[3],
            $refined_clusters[$fi]->[4],
            $refined_clusters[$fi]->[5],
            $refined_clusters[$fi]->[-1],
	    "total Hitsize:".$refined_clusters[$fi]->[6],
	    "SLICE:".$slice->start."-".$slice->end,
	    $containerSize."-".$midpt,
   	    "q_range:".($refined_clusters[$fi]->[5] - $refined_clusters[$fi]->[4]),	
	    "h_range:".($refined_clusters[$fi]->[3] - $refined_clusters[$fi]->[2]) )."\n";
  }
} # end if (0) block.
...
sub _convert_GenomicAlignBlocks_into_DnaDnaAlignFeatures {
  ...
  foreach my $this_genomic_align_block (@$genomic_align_blocks) {
    ...
    my $ga_cigar_line;
### wc2:: Note ###
# In order to deal with large alignments due to assemblies of identicial path/nature.  Many changes were made.
#  The code below which analyses the cigar string, is a piece of code that causes the machine to run out of memory,
#  when hitting these regions.  As this code is not important in the browser, this will be turned off.
if(0){
    do {
