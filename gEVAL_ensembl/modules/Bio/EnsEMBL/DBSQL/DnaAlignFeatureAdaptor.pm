...
=head2 store
  ...
  gEVAL Updated code (wc2 2014):
    -two things gEVAL has that ensembl doesn't and thats a dna_align_feature_attrib table, and the use of the extra/external data column in 
    dna_align_feature table.
    -these are things changed compared to the standard ensembl code.
    -modified using the e73 version of the DnaAlignFeatureAdaptor.pm code 
...
sub store {
  ...
  my $sth = $self->prepare(
    "INSERT INTO $tablename (seq_region_id, seq_region_start,
                             ...
                             external_data)       
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"    # 17 arguments with the last (external data) added for gEVAL.
  );
...
FEATURE:
  foreach my $feat (@feats) {
    ...
    my $ext_data   = $feat->extra_data || ""; # gEVAL specific for storing extra info.
    ...
    $sth->bind_param( 17, $ext_data,             SQL_LONGVARCHAR);   # gEVAL bind param for ext info.
...
=head2 update
    ...
    gEVAL specific used to update the DnaAlignFeatures.
    wc2 2014
=cut
...
sub update {
  ...
  # gEVAL specific Add external_data field.
  my $sth = $self->prepare(qq{UPDATE $tablename set seq_region_id=?, seq_region_start=?, seq_region_end=?, seq_region_strand=?, hit_start=?, hit_end=?, hit_strand=?, hit_name=?, cigar_line=?, analysis_id=?, score=?, evalue=?, perc_ident=?, external_db_id=?, hcoverage=?, pair_dna_align_feature_id=?, external_data=? where dna_align_feature_id=?});
 ...
 FEATURE: foreach my $feat ( @feats ) {
    ...
    my $ext_data   = $feat->extra_data || "";  #gEVAL specific
    ...
    $sth->bind_param(17,$ext_data);   #gEVAL specific
    $sth->bind_param(18,$feat->dbID);
  ...
  while ( $sth->fetch() ) {
	...
	$seq_region_strand *= -1;
      ...
      if ( (    $seq_region_end < 1
	   ...
	   # PATCH MAYHEM !!!!  
	   #  Been getting features skipped on the patch/hap/alt regions, almost randomly.
	   #  I'm hoping adding this little bit will solve it.
	   #  if you turn on the "testing purposes" below, you should be able to see whats going on.
	   # wc2@sanger.ac.uk
	   ( ($slice->seq_region_name ne $dest_slice->seq_region_name) &&
	     ($dest_slice->is_reference() ) ) )
      {
    ...
    push( @features,
          $self->_create_feature_fast(
             'Bio::EnsEMBL::DnaDnaAlignFeature', {
               ...
               'hcoverage'      => $hcoverage,
               'extra_data'     => $extra_data || undef, #$evalled_extra_data,
